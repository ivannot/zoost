...
| [`docs/architecture.md`](docs/architecture.md) | before changing module boundaries, pull lifecycle, mirror writes or dependency roles |
| [`docs/operations.md`](docs/operations.md) | before transferring Cloudflare, GitHub, Store or canary ownership to another maintainer |
...
| [`docs/usability.md`](docs/usability.md) | before changing first-run guidance or the sample path - the five-session protocol, and why five people locate friction and measure nothing |
...
somebody needs it - which is how `release.sh` refused every run for weeks over a line nobody could
act on. **A check that runs rarely gets both proofs on the day it is written.**

**Everything else about this suite is in [`docs/testing.md`](docs/testing.md)** - what the lifters can
and cannot reach, why the panels are not restructured to be importable, how a test can fail
unreadably, the traps in mutating a file to prove a check, and the message-fold that `panel.test.mjs`
holds. Read it before writing a test or a checker.
