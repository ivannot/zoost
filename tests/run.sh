...
# **Does every shipped script still parse?** Asked first, because it is the cheapest question here
# and because a file that does not parse makes every later answer meaningless - the panel loads
# nothing, and a unit test that lifts one function out of it passes regardless.
#
...
checked=0
for f in apps/*/*.js; do
  node --check "$f" || { echo "$f does not parse - nothing in this suite can be trusted about it" >&2; exit 1; }
  checked=$((checked + 1))
done
echo "  parse: $checked shipped script(s), all of them valid JavaScript"
