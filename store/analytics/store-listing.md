...
```
Zoost mirrors a Zoho Analytics workspace into plain local files, then draws you the map that Analytics never shows you.

A workspace that has been alive for a few years is hundreds of views. Some are tables, some are query tables with SQL somebody wrote at 2am, most are reports built on top of reports. Analytics will tell you what a single view depends on if you ask it, one view at a time. It will not tell you the shape of the whole thing, it will not tell you what changed last month, and it will not tell you which of those hundreds of views nothing reads any more.

WHAT IT DOES

- Local mirror, your Git. One pull writes the whole workspace to disk: the view census, the column structure of every table and query table, the relations, and one .sql file per query table. Commit it and git diff answers "what changed in this workspace last month" — a question Analytics cannot answer at all.
...
- Foreign keys on every column, and they are links. Each column shows what it points at and what points at it, both directions, because a foreign key is not symmetric. Click one and you are looking at the other table. Taken from the ER model Analytics itself draws, with the join written exactly as Zoho writes it.
...
- What nothing depends on. Ask Analytics its own dependency question for every view, and see which ones nothing in the workspace reads. Stated as candidates, never as a verdict: a shared link, a scheduled export, an embedded report or an API consumer is invisible to that graph, and Zoost says so next to the number.

- Design and data, told apart. Analytics tracks when a view's design last changed separately from when its data last refreshed. A report whose data refreshed nightly for two years but whose design nobody has touched since 2019 is a very different object from one redesigned last week, and the list shows both.
...
- Exports you can hand over. The whole workspace as one self-contained HTML file, or as Markdown shaped as context for an external AI tool — the Markdown carries the constraints of Analytics' own SQL dialect too, so an assistant that has never seen Analytics does not write queries that cannot run. A dialog decides section by section what the file may contain; the SQL is opt-in and flagged every time.
...
- It never writes to Zoho Analytics. No creating, editing or deleting a view, ever.
...
- It does not run, validate or deploy SQL. Whatever the assistant writes is a draft; Analytics is the only thing that can tell you it compiles.
...
- Lookups between base tables come from Analytics' own ER model. What that model does not draw, Zoost does not know.
...
Its single purpose is comprehension and versioning of a Zoho Analytics workspace's design. It reads only what the signed-in user can already see in the Analytics interface, using their existing session, and never modifies anything in Analytics and never reads the data rows inside tables.
```
...
```
The extension's entire interface is a side panel shown beside the Zoho Analytics tab. The panel has to stay visible while the user navigates Analytics, because the workspace it acts on is whichever one the active tab is in — the panel reads the workspace id from the tab's URL and disables every Zoho-bound action when the tab moves to a different workspace. A popup, which closes on every click, cannot do that. No other surface is used.
```
...
```
The extension reads the workspace through Zoho Analytics' own endpoints, which are only reachable from a page on the Analytics origin with the user's session cookies. A content script on that origin performs those authenticated reads and hands the results to the side panel.

chrome.scripting is used for one thing: re-injecting that content script into the Analytics tab when it is not present — after a navigation, or when the panel is opened on a tab that was already loaded. Without it the panel would work only on tabs opened after the extension started. No code is ever injected into any other site, and nothing is injected that is not part of this extension's own package.
```
...
```
https://analytics.zoho.eu/*, .com, .in, .com.au, .jp and analytics.zohocloud.ca — the Zoho Analytics data centres. The extension reads the workspace's structure from Analytics' own endpoints on whichever of these the user's account lives on. It cannot know which one in advance, and the set is exactly the list of Analytics regions, no wider.
