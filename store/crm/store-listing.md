...
**Before the next Zoho CRM submission: §9 has to be pasted again, and `tools/submitted.py crm` must not run until it has been.** The host justification was rewritten after 1.39.0 went in (`2ddf111`, on 11 August: it argues why the extension needs those hosts instead of enumerating the data centres, which the manifest already does), so what Google is reviewing is the previous text. This app has no recorded baseline yet: `store/crm/listing.json` is absent, and `python3 tools/storecopy.py crm --changed` reports each field as new - so the first run of `submitted.py` writes the baseline from this file as it stands. Run it early and it records sections Google has not seen, §9 among them, and `--changed` will report them as already pasted from then on. Delete this paragraph once §9 is actually in the dashboard.
