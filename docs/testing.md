...
**The endpoint pull has separate browser subjects, because one fake cannot honestly stand for both
extensions.**
...
That split found a real boundary defect on its first run. In the Analytics HAR, all **83 of 83** `editsql`
...
the boundary, the same seven raw routes produce the seven-file mirror. CRM adds a second measured
boundary: when the first Connections request answers `INVALID_CSRF_TOKEN`, its probe replays the
three Deluge bootstrap reads observed in `crm2.har` and verifies that the retry writes the catalogue.
The `INVALID_CSRF_TOKEN` failure observed in `crm.har` is modelled as the negative case, not
mistaken for an empty connections list. What this still does not claim is equally explicit: the
Chrome message bus and
File System Access implementation are adapters in these runs, and the session is not a real Zoho
account.
