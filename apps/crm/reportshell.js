...
.ftbl td{padding:5px 10px;border-bottom:1px solid var(--line);overflow-wrap:anywhere}.ftbl td.mono{color:var(--accent)}
...
function reportHead(subject, metaLines, filterPlaceholder, made) {
  return `<header><h1>${escReport(subject)}</h1>`
...
const REPORT_FILTER_JS = "function filt(){var q=document.getElementById('q').value.trim().toLowerCase();document.querySelectorAll('[data-name]').forEach(function(s){s.style.display=(!q||s.dataset.name.indexOf(q)>=0)?'':'none';});}"
