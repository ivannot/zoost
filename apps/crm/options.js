...
async function onPickRoot() {
  try {
    ...
    // Not «reopen the window»: that sentence was written for a settings page in a window of its
    // own, and there is nothing to reopen now - the panel is behind this form and picks the
    // folder up from `settingsStamp` by itself. Telling the reader to do something that is
    // neither necessary nor possible is worse than saying nothing.
    toast('Working folder set. Close the settings to see the workspaces.');
