...
function select(id, nopush) {
  ...
  // **A missing value is an absent badge, never the word «undefined».** `category` was guarded and
  // its two neighbours were not, so a node without a namespace - which a graph built by an older
  // writer, or read from a fixture that predates one, produces - printed `undefined` in a pill and
  // again under the title. An interpolation with no guard is a defect waiting for the day the shape
  // changes; there is no reading of this pane where that word is the right answer.
  const badges = schema
    ? `${n.namespace ? `<span class="badge">${esc(n.namespace)}</span>` : ''}${n.unreadable ? '<span class="badge">not described by Zoho</span>' : ''}${n.dead_suspect ? '<span class="badge">unreferenced</span>' : ''}`
    : `<span class="badge">${esc(n.namespace || 'unknown')} \u00b7 ${esc(n.category || '')}</span>${n.rest ? '<span class="badge b-rest">REST</span>' : ''}${n.dead_suspect ? '<span class="badge">no caller</span>' : ''}`;
  ...
  $('gvmain').innerHTML = `
    ...
    ${n.display_name ? `<div class="sub">${esc(n.display_name)}</div>` : ''}
...
function erShowMaybeHeavy(after) {
  ...
  if (!erLaidOut && n >= SPIN_NODES) {
    runHeavy($('v-er'), `Laying out ${n} ${NOUN().n}\u2026`, draw);
...
// `DATA &&` because the chrome is put back *before* the graph is applied - the render reads the
// search box, so clearing it afterwards would filter the list by a search nobody can see. With no
// graph yet the word is the one the previous projection had; `openGraphView` paints it again once
// `DATA` is there, which is the only moment the answer can be right.
const erEmphLabel = () => (erEmph === 'relations' ? (DATA && DATA.kind === 'schema' ? 'relations' : 'edges')
                                                 : (DATA && DATA.kind === 'schema' ? 'modules' : 'calls'));
$('erEmph').onclick = () => {
 ...
 *  an item from the other projection, `Fields: key` was greyed out for the life of the window
 *  because only its click handler ever wrote `disabled`, `Emphasis` read «modules» while still drawn
 *  pressed, and the Layout popover was open over a drawing it had never been opened on.
 *
...
function syncGraphChrome() {
  ...
  // written back rather than left describing a function from the projection before this one.
  const mainEl = $('gvmain');
  if (mainEl) mainEl.innerHTML = '<div class="empty">Select a function.<br>Left = who calls it (impact if you change it). Right = what it calls.</div>';
