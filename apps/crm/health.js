...
async function buildHealth(op = beginWorkspaceOp()) {
  ...
  const mirror = short === null || short === undefined
    ...
    : short > 0
      ...
      : '';
  // **And the other absence, which is not a failure.** A function in another language is listed and
...
async function openHealth() {
  ...
  if (!dir) return;
  closeAI(); closeOverview(); navShow(false);   // one panel at a time - the history included
...
// `blueprint` was missing, and Zoho does send it: a function run by a blueprint carries one in its
// `associated_place`, so the name drew as plain text - a process the reader could see and could not
